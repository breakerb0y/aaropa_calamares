...
static Qt::CheckState parentCheckState(OptionTreeItem* parent) {
  if (parent) {
...
OptionTreeItem::OptionTreeItem(const QString& optionName,
...
OptionTreeItem::OptionTreeItem(const QVariantMap& groupData, OptionTag&& parent)
    : m_parentItem(parent.parent),
      ...
      m_isHidden(isHiddenException() ||
                 Calamares::getBool(groupData, "hidden", false)),
      ...
      m_input(Calamares::getBool(groupData, "editable", false) ? "Value..."
                                                               : ""),
...
OptionTreeItem::~OptionTreeItem() { qDeleteAll(m_childItems); }

void OptionTreeItem::appendChild(OptionTreeItem* child) {
  m_childItems.append(child);
}

OptionTreeItem* OptionTreeItem::child(int row) {
  return m_childItems.value(row);
}

int OptionTreeItem::childCount() const { return m_childItems.count(); }

int OptionTreeItem::row() const {
  if (m_parentItem) {
    return m_parentItem->m_childItems.indexOf(
        const_cast<OptionTreeItem*>(this));
  }
  return 0;
}

QVariant OptionTreeItem::data(int column) const {
  switch (column) {
    case 0:
...
OptionTreeItem* OptionTreeItem::parentItem() { return m_parentItem; }

const OptionTreeItem* OptionTreeItem::parentItem() const {
  return m_parentItem;
}

bool OptionTreeItem::hiddenSelected() const {
  if (!m_isHidden) {
...
bool OptionTreeItem::isHiddenException() const {
...
void OptionTreeItem::setSelected(Qt::CheckState isSelected) {
...
void OptionTreeItem::updateSelected() {
...
void OptionTreeItem::setChildrenSelected(Qt::CheckState isSelected) {
...
void OptionTreeItem::selectChildren(QString optionName) {
  if (!isDistinct()) {
    return;
  }
...
void OptionTreeItem::removeChild(int row) {
  if (0 <= row && row < m_childItems.count()) {
    m_childItems.removeAt(row);
  } else {
    cWarning() << "Attempt to remove invalid child in removeChild() at row "
               << row;
  }
}

int OptionTreeItem::type() const { return QStandardItem::UserType; }

QString OptionTreeItem::toOperation() const { return m_description + m_input; }

bool OptionTreeItem::operator==(const OptionTreeItem& rhs) const {
  ...
  if (isGroup()) {
    return name() == rhs.name() && description() == rhs.description() &&
           preScript() == rhs.preScript() && postScript() == rhs.postScript() &&
           m_showReadOnly == rhs.m_showReadOnly &&
